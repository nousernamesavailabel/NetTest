...
class JitterRunner:
    ...
    def run(self, src_ssh: SSHManager, dst_ssh: SSHManager, dst_host: str) -> JitterResult:
        ...
        dst_ssh.run(f"pkill -f 'iperf3 -s' || true", timeout=10)
        time.sleep(0.5)
        dst_ssh.run_background(f"iperf3 -s -p {p.iperf3_port} --one-off")
        time.sleep(1)
...
class LatencyUnderLoadRunner:
    ...
    def run(self, src_ssh: SSHManager, dst_ssh: SSHManager,
        ...
        dst_ssh.run(f"pkill -f 'iperf3 -s' || true", timeout=10)
        time.sleep(0.5)
        dst_ssh.run_background(f"iperf3 -s -p {self.iperf3_port} --one-off")
        time.sleep(1)
